...
import { getAuthorizedHeaders } from './authSession.js';

const API_BASE = `${import.meta.env.VITE_API_BASE_URL || ''}/api/v1/inehss`;
...
export interface FormTemplate {
    ...
    map_icon?: 'warning' | 'alert' | 'biohazard' | 'radiation' | 'info';
    map_color?: string;
...
export async function getMyAssignments(token: string): Promise<OfficerAssignment[]> {
    const headers = await getAuthorizedHeaders(token);
...
export async function uploadAttachment(
    ...
    const headers = await getAuthorizedHeaders(token, { 'Content-Type': 'multipart/form-data' });
